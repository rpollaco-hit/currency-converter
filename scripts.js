const CONFIG = {
  OER_APP_ID: (window.APP_CONFIG && window.APP_CONFIG.OER_APP_ID) || "",
};
...
async function convert() {
  ...
  try {
    ...
    toValue.textContent = "—";
    setRateInfo("");
    setStatus("Unable to calculate conversion right now.");
  }
}
...
async function copyConvertedValue() {
  ...
  try {
    ...
    setStatus("Converted value copied.");
    setTimeout(() => {
      if (statusMessage?.textContent === "Converted value copied.") setStatus("");
    }, 1800);
  } catch {
    setStatus("Could not copy the value.");
  }
}
...
async function init() {
  ...
  if (DEMO_MODE) {
    setStatus("No API key detected. Running in Demo Mode.");
  }
...
document.addEventListener("DOMContentLoaded", () => {
  init().catch((error) => {
    ...
    setStatus("Application could not be initialized.");
  });
});
